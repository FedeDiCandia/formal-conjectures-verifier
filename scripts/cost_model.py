...
def load() -> dict:
    ...
    idx = {}
    # `open_refutable` is the only row that has to READ the source of each theorem,
    # and a theorem's source lives in the snapshot its index was built from. The
    # archive root is therefore switched for each index: with a single root the
    # ranges of one index would be applied to the other snapshot's files, which is
    # how this row once came out wrong.
    for name, perc, archive in (
            ("bench-v1", "verifier/problem_index.json",
             ROOT / "external" / "formal-conjectures"),
            ("main", "verifier/problem_index_main.json",
             ROOT / "external" / "fc-main")):
...
SCENARIOS = {
    "A": {
        "name": "direct Lean proofs of open statements",
        "optimistic": (0.02,
    ...
    "C": {
        ...
        "optimistic": (None, "derived from A and B"),
        "realistic": (None, "derived from A and B"),
        "pessimistic": (None, "derived from A and B"),
    },
}
...
# Strategy B: how many targets really exist.
N_SELECTED_HUNT = 30          # MEASURED: runs/hunt/selection.json
REACHABLE_FRONTIER_FRACTION = 0.50   # ESTIMATED from 2 of the 4 literatures checked
...
def report_text(data: dict) -> str:
    ...
    p(f"**MEASURED** — {len(problems)} problems attempted, {len(solved)} solved, "
      ...
      f"the agent.")
    p()
    p("| level | proof lines | outcome | iter. | expl. | verifications | seconds | cost |")
    p("|---|---|---|---|---|---|---|---|")
    for x in problems:
    ...
    p()
    p("**MEASURED** — the four `easy` entries are problems in the `test` "
      "category, that is sanity checks written by the archive's own authors: "
      "4 out of 4. The seven `medium` and `hard` entries are variants of real "
      "conjectures, in the `research solved` category: 5 out of 7. "
    ...
    p(f"**MEASURED** — cost of the FIRST iteration, median over {len(problems)} "
      ...
      f"the mixed strategy of section 4.")
    p()
    ...
    p()
    p("Formula: (cost if solved x p + cost if failed x (1-p)) / p, that is how much "
...
def report_part_two(data: dict, r: list[str]) -> list[str]:
    ...
    p()
    p(f"The `main` counts hold under the default semantics "
      f"`google.answer = always_true`, in which `answer(sorry) ↔ P` becomes `True ↔ P` "
      f"and the problem really is provable. Until this session the snapshot also "
      f"declared a second library that compiled the same files with `postpone`, and the "
      f"two sets of `.olean` files overwrote each other: under that semantics those "
      f"{idx['main']['open_refutable']} problems are not attackable at all. The surplus "
      f"library has been disabled — "
    ...
    p(f"Two observations that change the strategy. First: **none** of the "
      f"{idx['main']['open_problems']} problems marked `research open` has a "
      f"complete proof in the archive — the archive is consistent with itself, "
    ...
    # ---------------------------------------------------------------- 4
    p("## 4. Projection onto the open problems")
    p()
    ...
    p(f"- **A, full attempt** on an open statement: **${c_a:.2f}** — "
      ...
      f"(${failed[0]['cost']:.4f} stopped by the 20 iterations, "
    ...
    for sc in ("optimistic", "realistic", "pessimistic"):
        ...
        p(f"- **{sc}: p = {prob:.2%}** — ESTIMATED. {reason}.")
    p()
    ...
    p(f"Here the limit is not money: it is the number of problems on which a search "
      ...
      f"Selfridge: checked only up to about k = 29) and two do not (Erdos 366: "
      f"checked up to 10^22; Goldbach and Legendre: up to 4x10^18). Two out of "
      f"four, 90% Clopper-Pearson interval "
      f"{clopper_pearson(2,4)[0]:.0%} – {clopper_pearson(2,4)[1]:.0%}: "
      f"**ESTIMATED** {reachable_share:.0%}, so about "
      f"**{n_b:.0f} real targets**.")
    p()
    p(f"API cost to saturate the strategy: {n_b:.0f} x ${c_b:.2f} = "
      f"**${n_b*c_b:.2f}**. Machine time: with 8 searches in parallel, "
      f"about {n_b/8:.0f}-{n_b/4:.0f} nights.")
    p()
    p("| spend | searches | optimistic | realistic | pessimistic |")
    p("|---|---|---|---|---|")
    for b in SPEND_LEVELS:
        n = min(b / c_b, n_b)
        note = " *(saturated)*" if b / c_b > n_b else ""
    ...
    p(f"- selected attempts are worth {AMPLIFICATION['optimistic']:.0f}x / "
      ...
      f"the signal exists but is imperfect.")
    p(f"- **MEASURED** — covering all {n_targets} verifiable open problems with a "
      f"single shot each costs **${n_targets*c_s:.0f}**.")
    p()
...
def report_part_three(data: dict, r: list[str]) -> list[str]:
    ...
    p()
    reg = ROOT / "runs/hunt/euclid_squarefree/search.log"
    if reg.is_file():
        last = None
    ...
    if probe:
        ...
        p(f"**MEASURED** — an automatic probe over {n} discrete open statements "
          f"(`decide`, `plausible`, `norm_num`, `simp_arith`, in "
          f"direct and negated form, {8*n} attempts in all): **{len(notable)}** produced "
          f"anything. 90% Clopper-Pearson interval on the "
          f"fraction of open problems that fall on their own: {low:.1%} – {high:.1%}.")
        for v in notable:
            p(f"  - `{v['problem']}`: {v.get('ATTENTION', 'to be examined')}")
        if not notable:
    ...
    p(f"**The mixed strategy (C), and below ${1.0*c_c*n_targets:,.0f} there is no "
      f"reason to do anything else.** Three reasons, in order of weight.")
    p()
    p(f"1. *The sieve costs almost nothing and covers everything.* A single shot at "
      ...
      f"No other spending in this project buys as much information per dollar.")
    p()
    ...
    p()
    def expected_c(b: float, scen: str) -> float:
    ...
    def triple(b: float) -> str:
        return " / ".join(fmt(expected_c(b, sc)) for sc in
                          ("optimistic", "realistic", "pessimistic"))

    p(f"- **$53** — the complete sieve: one shot at all "
      f"{n_targets} verifiable open problems. Expected successes {triple(53)} "
      f"(optimistic / realistic / pessimistic). It is worth doing even if you "
      f"expect zero successes: what you buy is the map of where the model has a "
      f"plan.")
    p(f"- **$174** — the complete sieve plus full attempts on the best "
      f"{DIVE_SHARE:.0%}. Expected successes {triple(174)}. This is the point where, if "
    ...
    p(f"- **$500** — expected successes {triple(500)}. Worth it only if the "
      f"$53 sieve showed promising targets: spent blindly, "
      f"it buys attempts on problems where the model had no plan at all.")
    p(f"- **$1000–$5000** — expected successes {triple(1000)} and {triple(5000)}. "
    ...
    p("**A recommendation about targets, not only about spending.** The "
      f"{idx['main']['solved_without_proof']} problems marked `research solved` "
...
def wrap(lines: list[str], width: int = 80) -> list[str]:
    """Wrap the prose at `width`, leaving tables, headings and code untouched.

    The report is meant to be read in the repository, so the paragraphs are wrapped
    here rather than by hand afterwards: a generated file that nobody edits after
    generation stays honest.
    """
    out: list[str] = []
    for line in lines:
        ...
        if line.startswith(("- ", "* ")):
            ...
            out += textwrap.wrap(line, width=width, break_long_words=False,
