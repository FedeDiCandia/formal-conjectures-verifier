...
l'agent **non consegna candidati**: calcola, capisce dov'è la difficoltà, e si
...
senza Lean e senza tools, e poi si fa a pezzi da un revisore severo. Se
...
from agent import carica_env          # noqa: E402
from costs import Budget, LimiteSpesaSuperato   # noqa: E402
...
def messaggio_autore(p, indice) -> str:
    ...
    riceve l'agent — il file dell'archivio con le dimostrazioni nascoste — che
    contiene le definizioni, i termini di prova dei primi valori, e il commento
    della fonte.
    """
    from hide import file_senza_dimostrazioni
...
def main() -> int:
    ...
    import agent
    sys.stdout = agent._Doppio(sys.stdout, percorso_registro)
