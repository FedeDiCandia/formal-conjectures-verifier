#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# Prepara un secondo snapshot dell'archivio, preso da un commit FISSO di `main`.
#
# PERCHE'
# Il tag di benchmark `bench-v1-lean4.27.0` e' del 2026-05-06, cioe' proprio
# alla data di taglio dell'addestramento di claude-opus-5 (maggio 2026). Ogni
# dimostrazione contenuta in quel tag era quindi pubblica su GitHub prima
# dell'addestramento: calibrare un agente su quei problemi misura anche quanto
# il modello ricorda, non solo quanto sa dimostrare.
#
# Su `main` ci sono 548 file di problemi aggiunti dopo il 1 giugno 2026. Quelli
...
#: Commit fisso del ramo main. Scelto il 2026-09-10.
...
# Un worktree condivide la cartella .git con il clone esistente: risparmia
# circa un gigabyte e il tempo di un secondo clone.
if [ ! -d "$SNAP" ]; then
...
echo
echo "  olean compilati: $(find "$SNAP/.lake/build/lib/lean/FormalConjectures" -name '*.olean' 2>/dev/null | wc -l)"
