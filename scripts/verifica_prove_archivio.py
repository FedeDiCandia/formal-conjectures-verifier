...
Un problema entra nella calibrazione di un agente SOLO se la sua dimostrazione
d'archivio viene ACCETTATA qui. Altrimenti chiedere a un agente di risolverlo
...
def lavora(i, p):
    ...
    try:
        t0 = time.time()
        try:
            estratto = prova_archivio.estrai(p, idx)
        except prova_archivio.NonEstraibile as e:
