...
## 2. Riduzioni a problemi aperti noti

| problema | riduzione affermata | stato in letteratura |
|---|---|---|
| **`OeisA108569.conjecture`** ([A108569](https://oeis.org/A108569)) — «tutti i termini oltre il primo sono pari», cioè: nessun k > 1 dispari con φ(k) = φ(k+φ(k)) | Un tale k, se è un primo p, forza m = 2p−1 a essere un **numero di Lehmer di indice 2**: composto, senza quadrati, con φ(m) \| m−1 di quoziente 2, non divisibile per 3, tutti i fattori primi ≥ 5 e almeno 7 di essi, e p ≡ 1 (mod 24). Quindi il caso residuo **è** un'equazione di tipo Lehmer | La congettura è di **Farideh Firoozbakht** (luglio 2005). La voce OEIS **non menziona Lehmer** in nessun commento, formula, riferimento o rimando (i soli rimandi sono A005384 e A051487) — *fatto accertato sulla voce*. Una ricerca in rete non trova il collegamento: la letteratura vicina riguarda φ(n) = φ(n+k) e φ(n+k) = Mφ(n), equazioni diverse — *prova debole*. Il problema di Lehmer è aperto dal 1932; per una soluzione si sa n > 10²⁰ e ω(n) ≥ 14 (Cohen–Hagis 1980) |

Altre righe si aggiungono quando la fase D finisce: il giro è in corso su 19
problemi e finora ha prodotto `PARTIAL` o `NO-PROOF` su tutti gli altri, che è
l'esito atteso e non fa notizia. Le riduzioni interessanti sono quelle dove il
modello **nomina** il problema noto a cui si arriva, come qui.
