...
% Compilato con XeLaTeX (tectonic). Rispetto al preambolo proposto, le righe pensate
% per pdflatex sono sostituite dai loro equivalenti Unicode:
%   fontenc T1 + inputenc utf8  -> tolti: con XeTeX non servono
%   newtxmath[libertine]        -> unicode-math con Libertinus Math
%   beramono                    -> DejaVu Sans Mono, stesso disegno di Bera (Vera) Mono
%                                  ma con i simboli Unicode del codice Lean
% Le definizioni dei teoremi stanno dopo cleveref: prima, \cref chiama "theorem"
% anche lemmi e osservazioni.
...
% DejaVu Sans Mono non ha ∣ (U+2223): quel solo carattere viene da Noto Sans Mono
...
% Segno di a capo: senza, una riga spezzata sembra una riga nuova con un'altra
% indentazione, che in Lean ha un significato.
...
% Con XeTeX, listings fa passare dal suo buffer solo i caratteri sotto 256; gli altri
% sono stampati appena letti, prima della parola in sospeso ("⟨m, rfl⟩" usciva come
% "⟨m, ⟩rfl"). Qui i caratteri non ASCII dei file Lean sono registrati come listings
% registra quelli a 8 bit.
\makeatletter
...
% --- valori presi dal rapporto di verifica; file allegati al PDF ------
...
\begin{tikzpicture}[x=15mm, y=8mm,
    term/.style={draw, minimum width=12.5mm, minimum height=7mm, inner sep=0pt},
    semi/.style={term, fill=black!13, line width=0.9pt},
    odd/.style={term, dashed},
  ...
  \foreach \k/\v/\p/\q/\st in {1/24/2/12/term, 2/33/3/11/semi, 3/40/4/10/term,
    ...
    \node[\st] at (\k,0) {$\v$};
    \node[idx] at (\k,1) {$T_6{+}\k$};
    \node[idx] at (\k,-1) {$\p\cdot\q$};
  }
  \node[idx] at (0,1) {$T_6$};
  \node[idx] at (7,1) {$T_7$};
  \node[note] at (0,-1) {$2n{+}1$};
  \node[note] at (7,-1) {$2n{+}3$};
  \node[note, anchor=east] at (-0.55,1) {index};
  \node[note, anchor=east] at (-0.55,0) {term};
  \node[note, anchor=east] at (-0.55,-1) {$p\cdot q$};
  \draw[decorate, decoration={brace, mirror, amplitude=5pt}]
    (0.6,-1.45) -- (6.4,-1.45)
    node[midway, below=7pt, draw, rounded corners=2pt, inner sep=3pt]
      {$p + q = 2n+2 = 14$};
\end{tikzpicture}
\caption{Antidiagonal $n = 6$ of A105020, from index $T_6 = 21$ to $T_6 + 6 = 27$,
and the first term of antidiagonal $7$, at $T_7 = 28$. By Lemma~\ref{lem:closed} the
term at $T_6 + k$ is $pq$ with $p = k+1$ and $q = 2n+1-k$, so $p + q = 2n+2 = 14$
throughout. Dashed: the odd terms $a(T_6) = 13$ and $a(T_7) = 15$, with $n = 6$ terms
between them. Shaded: the semiprimes among those $n$ terms, $33 = 3 \cdot 11$ and
$49 = 7 \cdot 7$, one for each Goldbach partition $14 = 3 + 11 = 7 + 7$.}
...
From OEIS A045917~\cite{oeis-a045917}:
\begin{lstlisting}
