#!/usr/bin/env python3
"""
Un agent minimo che tenta di dimostrare un problema dell'archivio.
...
E' un ciclo semplice:
  ...
  4. si ripete finche' il modello smette di chiedere tools, oppure finche'
     `lean_check` accetta una dimostrazione, oppure finche' finiscono i soldi
     o i tentativi.

Gli tools disponibili sono due (vedi agent/tools.py):
...
chiamata si controlla il saldo: se e' esaurito, l'agent si ferma e lo dice.
"""
...
from costs import Budget, LimiteSpesaSuperato, Consumo   # noqa: E402
...
from hide import file_senza_dimostrazioni, controlla_che_sia_nascosta  # noqa: E402
...
#:
#: PERCHE' ESISTE. Nel primo giro sui problemi aperti l'agent ha fallito dieci
...
#: viene dalla difficolta' dei problemi o dal modo in cui l'agent si arrende?
...
@dataclass
class Iterazione:
    """Una passata del ciclo: una chiamata all'API piu' gli tools usati."""
    numero: int
    token_input: int = 0
    token_output: int = 0
    cache_scritta: int = 0
    ...
    def classifica_fallimento(self) -> str:
        ...
        Regola dichiarata, non a sensazione:
          * se non c'e' stato nessun tentativo vero (tutte le verifiche erano
            esplorazioni), il fallimento e' di SISTEMA: l'agent non e' arrivato
            a provarci, ha speso tutto a capire gli tools e l'API di Mathlib;
...
#: A quanti caratteri si riducono i risultati degli tools piu' vecchi.
...
def compatta_conversazione(messaggi: list, *, intatti: int = MESSAGGI_INTATTI,
    ...
    """Accorcia i risultati degli tools piu' vecchi. Ritorna quanti ne accorcia.
...
def risolvi(problema: Problem, indice: ProblemIndex, *, client, modello: str,
    ...
    strumenti_api = [tools.SCHEMA_LEAN_EXPLORE, tools.SCHEMA_LEAN_CHECK,
    ...
    for iterazione in range(1, max_iterazioni + 1):
        ...
        if not chiamate:
            t.motivo = "il modello ha smesso di usare gli tools senza una prova accettata"
        ...
        for chiamata in chiamate:
            if chiamata.name == "lean_explore":
                ...
                t0 = time.time()
                uscita = tools.esegui_lean_explore(
                ...
                t0 = time.time()
                rapporto, ok = tools.esegui_lean_check(
                ...
                t0 = time.time()
                uscita = tools.esegui_run_python(
...
def main() -> int:
    ...
    ap.add_argument("--registro", default=None,
                    help="dove scrivere il registro riga per riga (predefinito: "
                         "runs/lavori/agent-<data>.log). Serve per seguire il "
    ...
    if not args.senza_veglia and sys.platform == "darwin":
        processo_veglia = awake.avvia(os.getpid())
        motivi = awake.controlla(processo_veglia)
        if motivi:
    ...
    for i, p in enumerate(elenco, 1):
        if processo_veglia is not None and not awake.alimentatore_collegato():
