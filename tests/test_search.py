...
def test_una_ricerca_arriva_in_fondo(tmp_path):
    r = search_module.Ricerca("prova_conta", PROGRAMMA_CONTA, cartella=tmp_path / "conta",
                               variabili={"FINO_A": 50})
...
"""
    r = search_module.Ricerca("prova_var", programma, cartella=tmp_path / "var",
...
def test_il_checkpoint_viene_scritto(tmp_path):
    r = search_module.Ricerca("prova_ckpt", PROGRAMMA_CONTA, cartella=tmp_path / "ckpt",
                               variabili={"FINO_A": 30})
...
def test_la_ripresa_riparte_da_dove_era_arrivata(tmp_path):
    ...
    r = search_module.Ricerca("prova_ripresa", PROGRAMMA_CONTA, cartella=cartella,
...
def test_riprendi_falso_ricomincia_da_capo(tmp_path):
    ...
    r = search_module.Ricerca("prova_dacapo", PROGRAMMA_CONTA, cartella=cartella,
...
def test_la_ricerca_non_ha_accesso_alla_rete(tmp_path):
    r = search_module.Ricerca("prova_rete", PROGRAMMA_RETE, cartella=tmp_path / "rete")
    esito = r.esegui(verboso=False)
...
def test_la_ricerca_non_scrive_fuori_dalla_sua_cartella(tmp_path):
    r = search_module.Ricerca("prova_scrittura", PROGRAMMA_SCRITTURA,
...
'''
    r = search_module.Ricerca("prova_librerie", programma, cartella=tmp_path / "lib")
    esito = r.esegui(verboso=False)
