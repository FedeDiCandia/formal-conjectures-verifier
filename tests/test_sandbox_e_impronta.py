...
2. L'impronta dell'archivio, confrontata prima e dopo ogni verifica, si accorge
...
def test_l_impronta_e_stabile():
    a = modulo_impronta.calcola(config.ARCHIVE)
    b = modulo_impronta.calcola(config.ARCHIVE)
    assert a.n_file_contenuto > 100, "mi aspetto centinaia di file dell'archivio"
    assert not modulo_impronta.confronta(a, b), \
...
def test_l_impronta_rileva_un_file_modificato():
    ...
    differenze = modulo_impronta.confronta(a, b)
...
def test_l_impronta_rileva_un_file_cancellato():
    ...
    assert any("CANCELLATI" in d for d in modulo_impronta.confronta(a, b))
...
def test_l_impronta_rileva_un_cambio_nelle_dipendenze():
    ...
    assert any("Mathlib" in d for d in modulo_impronta.confronta(a, b))
...
def test_l_impronta_ignora_la_cartella_del_modulo_temporaneo():
    ...
    a = modulo_impronta.calcola(config.ARCHIVE, escludi=cartella.name)
    intruso = cartella / "S99.lean"
    intruso.write_text("-- file temporaneo\n", encoding="utf-8")
    try:
        b = modulo_impronta.calcola(config.ARCHIVE, escludi=cartella.name)
        assert not modulo_impronta.confronta(a, b)
    finally:
