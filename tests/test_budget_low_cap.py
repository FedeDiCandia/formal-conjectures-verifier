"""Con un tetto basso l'agent deve fare PIÙ DI UNA chiamata.
...
per problema l'agent aveva una chiamata sola, e su due problemi su undici ne ha
...
import agent
from costs import Budget, Consumo
...
def _chiamate_possibili(modello: str, tetto: float, token_input: int,
    ...
    for _ in range(50):
        ...
        max_tokens = b.max_tokens_sostenibile(token_input, agent.MAX_TOKENS,
                                              residuo=residuo)
        if max_tokens < agent.MIN_TOKENS_UTILI:
            break
        chiamate += 1
...
def test_la_soglia_vecchia_fermava_il_tentativo_prima_di_cominciare():
    ...
    max_tokens = b.max_tokens_sostenibile(16_287, agent.MAX_TOKENS, residuo=0.50)
    assert max_tokens < vecchia, "questi sono i numeri che fermavano il tentativo"
    assert max_tokens >= agent.MIN_TOKENS_UTILI, (
...
def test_il_limite_resta_rigido():
    ...
    for residuo in (0.02, 0.05, 0.12, 0.30, 0.50, 1.40, 2.00):
        for token_input in (2_000, 10_000, 40_000):
            mt = b.max_tokens_sostenibile(token_input, agent.MAX_TOKENS,
                                          residuo=residuo)
            if mt < agent.MIN_TOKENS_UTILI:
...
def test_sotto_la_soglia_minima_si_ferma():
    ...
    b = Budget(limite_dollari=5.0, modello="claude-fable-5-1")
    mt = b.max_tokens_sostenibile(10_000, agent.MAX_TOKENS, residuo=0.02)
    assert mt < agent.MIN_TOKENS_UTILI
...
def test_il_tentativo_interrotto_dal_budget_non_perde_il_lavoro_fatto():
    ...
    """
    from costs import LimiteSpesaSuperato
    t = agent.Tentativo(problema="X.y")
    t.iterazioni = 3
    t.verifiche = 1
