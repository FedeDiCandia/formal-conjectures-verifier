"""
Test del verifier full (verifier/verify.py).
...
E' state chosen perche':
  * l'archive ne contiene gia' one dimostrazione complete (quindi possiamo
    verificare che il verifier ACCETTI qualcosa, non only che rifiuti);
  * l'statement usa `jugglerStep`, one definition dichiarata nello stesso file:
    serve per il test "rifiuta chi ridefinisce one definition dell'archive";
...
comparator (slow, ma e' la garanzia vera). Ogni volta che entrambi si
applicano, qui li collaudiamo SEPARATAMENTE: con `run_guard=False` il filtro
testuale viene disattivato e resta only il giudizio di comparator. Se un giorno
...
def setup_module(module):
    """Salta all_items i test (con one explanation) se l'environment non e' ready."""
...
#: Le fixture adattate allo snapshot in uso, one volta sola per sessione.
...
def fixture(name: str) -> Path:
    ...
    riscrive la line di import al volo, cosi' la stessa suite gira su all_items e
    two gli snapshot.
    """
...
# --- 3. RIFIUTA one trial che aggiunge un assioma ---------------------------
...
# --- 4. RIFIUTA one trial che usa `native_decide` ---------------------------
...
def test_4b_rifiuta_native_decide_anche_senza_controllo_sintattico():
    ...
    # assioma con un name PER DICHIARAZIONE, del kind
    # `JugglerConjecture.jugglerStep_36._native.native_decide.ax_1_1`. E' la
    # ragione per cui il verifier work con one items di axioms PERMITTED e
    # non con one items di axioms vietati: un listing di names da vietare
    # avrebbe mancato la forma new senza dire niente.
...
def test_10_leans_info_messages_reach_the_author():
    ...
    successa durante il first shakedown con l'agent."""
...
def test_14_a_candidate_cannot_rewrite_a_file_of_the_archive(tmp_path):
    ...
    altera all_items quelle SUCCESSIVE, facendole confrontare la solution con un
...
'''), encoding="utf-8")
    ...
    assert target_hash() == before, \
        "il file compilato dell'archive E' STATO MODIFICATO: all_items le checks " \
